function Velocity_Resolve(init_vector,final_vel_vector,result_intensity)
...
result_c2_xy = zeros(Simulations_Amount,2);
    ...
    for i = 1:Simulations_Amount
        ...
        %check if intensity is larger than 0, if it is not then skip
        %simulation.
        if result_intensity(i) == 0
            result_c1_xy(i,:) = nan; %Set to NaN to avoid being plotted at (0,0)
            result_c2_xy(i,:) = nan; %Set to NaN to avoid being plotted at (0,0)
        else
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %save results multiplied by simulation intensity
            result_c1_xy(i,:) = [result_intensity(i)*x1 result_intensity(i)*y1];
            result_c2_xy(i,:) = [result_intensity(i)*x2 result_intensity(i)*y2];
